...
public class AdminMenuController {
    ...
    public void buttonOnClick(ActionEvent event) throws IOException {
        ...
        if (clickedButton == viewButton) {
            ...
            root = FXMLLoader.load(getClass().getResource("admin_leases.fxml"));
        }  else if (clickedButton == exitButton) {
            stage = (Stage) exitButton.getScene().getWindow();
            stage.close();
            Database.closeConnection();
        } else if(clickedButton==addPropertyButton){
            stage = (Stage) viewButton.getScene().getWindow();
            root = FXMLLoader.load(getClass().getResource("admin_add.fxml"));
        }
